...
export class Scene implements OnInit, AfterViewInit, OnDestroy {
  ...
  public ngOnInit(): void {
    this._subscription.add(
      this._gameEvents.start$.subscribe({
        next: () => {
          console.log(this._gameControl.getGame().type);

          if (this._gameControl.getGame().type === GameType.RUN) {
            this._fallingItems.length = 0;

            this._gameControl.resetHits();
          }
        },
      })
    );
